/* ═══════════════════════════════════════════════════════
   ml.js — محركات التعلّم غير المُشرف، مكتوبة من الصفر
   غابة العزل · K-Means++ · Autoencoder
   لا أي مكتبات خارجية. إعداد: محمود الباز فوزي قابيل
   ═══════════════════════════════════════════════════════ */
"use strict";
const ML = (() => {

  /* مولّد أرقام شبه عشوائية قابل لإعادة الإنتاج (Mulberry32) */
  ...
  /* التطبيع القياسي: صفر متوسط، انحراف معياري ١ */
  function zscore(M) {
  ...
  function buildTree(Z, idx, depth, maxD, rng) {
    ...
    const d = Z[0].length;
    for (let t = 0; t < d; t++) {                 // ابحث عن سمة قابلة للتقسيم
  ...
  function isolationForest(Z, opts = {}) {
    ...
    return Z.map(x => {
      ...
      return Math.pow(2, -(s / trees) / c);       // ∈ (0,1) والأقرب لـ١ أشد شذوذًا
    });
  }
  ...
  /* درجة اشتباه التجميع: بُعد عن المركز + عنقود ضيق */
