  ...
  return new Promise(function (res, rej) {
    ...
    fr.onload = function (e) {
      try {
        ...
        rej(new Error("تعذر قراءة الملف — تأكد أنه ملف جدول صالح."));
      }
    };
...
function normalizeRecords(raw) {
  ...
  if (!amtCol) throw new Error("لم أجد عمود المبلغ — حمّل النموذج الجاهز أو تأكد من وجود عمود «المبلغ».");
  var out = [];
...
function standardize(rows) {
  ...
  for (var j = 0; j < dim; j++) {
    var col = [];
    for (var i = 0; i < feats.length; i++) col.push(feats[i][j]);
    means.push(mean(col));
  ...
  for (var i = 0; i < feats.length; i++) {
    var row = [];
    for (var j = 0; j < dim; j++) row.push((feats[i][j] - means[j]) / stds[j]);
    out.push(row);
  }
  return out;
}
...
function iTree(data, depth, maxDepth) {
  ...
  if (!cols.length) return { size: data.length };
  var col = cols[Math.floor(Math.random() * cols.length)];
  var vals = data.map(function (r) { return r[col]; });
  var min = Math.min.apply(null, vals), max = Math.max.apply(null, vals);
  if (min === max) return { size: data.length };
  var split = rand(min, max);
  var leftData = data.filter(function (r) { return r[col] < split; });
  var rightData = data.filter(function (r) { return r[col] >= split; });
  return {
    split: split, col: col,
    left: iTree(leftData, depth + 1, maxDepth),
    right: iTree(rightData, depth + 1, maxDepth)
  };
}
...
function isolationForest(rows, contamination, nTrees) {
  ...
  nTrees = nTrees || 150;
  var rawFeats = buildFeatures(rows);
  var feats = [];
  for (var i = 0; i < rawFeats.length; i++) {
    var o = { idx: i };
    for (var j = 0; j < rawFeats[i].length; j++) o["f" + j] = rawFeats[i][j];
    feats.push(o);
  }
  ...
  for (var i = 0; i < feats.length; i++) {
    var total = 0;
    for (var t = 0; t < trees.length; t++) total += pathLen(feats[i], trees[t]);
    rawScores.push(Math.pow(2, -(total / trees.length) / c));
  }
  var sorted = rawScores.slice().sort(function (a, b) { return b - a; });
  var thIdx = Math.max(0, Math.floor(contamination * sorted.length) - 1);
  var th = sorted[thIdx] || 0.5;
  var minS = Math.min.apply(null, rawScores), maxS = Math.max.apply(null, rawScores);
  var out = [];
  for (var i = 0; i < rows.length; i++) {
    out.push({
      id: rows[i].id, label: rows[i].label, amount: rows[i].amount,
      score: ((rawScores[i] - minS) / (maxS - minS + 1e-9)) * 100,
      flag: rawScores[i] >= th
    });
  }
  return out;
...
function kmeans(points, k, maxIter) {
  ...
  var labels = [];
  for (var i = 0; i < points.length; i++) labels.push(0);
  for (var it = 0; it < maxIter; it++) {
    ...
    for (var i = 0; i < points.length; i++) {
      ...
      for (var c = 0; c < centroids.length; c++) {
        var d = 0;
        for (var j = 0; j < points[i].length; j++) d += (centroids[c][j] - points[i][j]) * (centroids[c][j] - points[i][j]);
    ...
    for (var c = 0; c < k; c++) {
      var cl = [];
      for (var i = 0; i < points.length; i++) { if (labels[i] === c) cl.push(points[i]); }
      if (!cl.length) continue;
      var newCentroid = [];
      for (var j = 0; j < cl[0].length; j++) {
        var colVals = [];
        for (var i = 0; i < cl.length; i++) colVals.push(cl[i][j]);
        newCentroid.push(mean(colVals));
      }
...
function kmeansAnomaly(rows, k) {
  ...
  for (var c = 0; c < k; c++) {
    var cnt = 0;
    for (var i = 0; i < labels.length; i++) { if (labels[i] === c) cnt++; }
    sizes.push(cnt);
  }
  var dists = [];
  for (var i = 0; i < feats.length; i++) {
    var d = 0;
    for (var j = 0; j < feats[i].length; j++) {
      d += (centroids[labels[i]][j] - feats[i][j]) * (centroids[labels[i]][j] - feats[i][j]);
    }
...
function autoencoderAnomaly(rows, epochs, lr) {
  ...
  function zeros(r, c) {
    var m = [];
    for (var i = 0; i < r; i++) { m.push([]); for (var j = 0; j < c; j++) m[i].push(0); }
    return m;
  }
  ...
  function forward(x) {
    var a1 = [], a2 = [], a3 = [], out = [];
    var i, j, s;
    for (i = 0; i < h1; i++) { s = b1[i]; for (j = 0; j < nIn; j++) s += W1[i][j] * x[j]; a1.push(Math.tanh(s)); }
    for (i = 0; i < h2; i++) { s = b2[i]; for (j = 0; j < h1; j++) s += W2[i][j] * a1[j]; a2.push(Math.tanh(s)); }
    for (i = 0; i < h1; i++) { s = b3[i]; for (j = 0; j < h2; j++) s += W3[i][j] * a2[j]; a3.push(Math.tanh(s)); }
    for (i = 0; i < nIn; i++) { s = b4[i]; for (j = 0; j < h1; j++) s += W4[i][j] * a3[j]; out.push(s); }
    return { a1: a1, a2: a2, a3: a3, out: out };
  }

  var N = feats.length;
  for (var e = 0; e < epochs; e++) {
    var gW1 = zeros(h1, nIn), gb1 = new Array(h1).fill(0);
    var gW2 = zeros(h2, h1), gb2 = new Array(h2).fill(0);
    var gW3 = zeros(h1, h2), gb3 = new Array(h1).fill(0);
    var gW4 = zeros(nIn, h1), gb4 = new Array(nIn).fill(0);
    ...
    for (var fi = 0; fi < feats.length; fi++) {
      ...
      var i, j, k2, s;
      var d4 = [];
      for (i = 0; i < nIn; i++) d4.push(2 * (f.out[i] - x[i]) / nIn);
      for (i = 0; i < nIn; i++) { gb4[i] += d4[i]; for (j = 0; j < h1; j++) gW4[i][j] += d4[i] * f.a3[j]; }
      var d3 = [];
      for (j = 0; j < h1; j++) { s = 0; for (i = 0; i < nIn; i++) s += d4[i] * W4[i][j]; d3.push((1 - f.a3[j] * f.a3[j]) * s); }
      for (j = 0; j < h1; j++) { gb3[j] += d3[j]; for (k2 = 0; k2 < h2; k2++) gW3[j][k2] += d3[j] * f.a2[k2]; }
      var d2 = [];
      for (k2 = 0; k2 < h2; k2++) { s = 0; for (j = 0; j < h1; j++) s += d3[j] * W3[j][k2]; d2.push((1 - f.a2[k2] * f.a2[k2]) * s); }
      for (k2 = 0; k2 < h2; k2++) { gb2[k2] += d2[k2]; for (j = 0; j < h1; j++) gW2[k2][j] += d2[k2] * f.a1[j]; }
      var d1 = [];
      for (j = 0; j < h1; j++) { s = 0; for (k2 = 0; k2 < h2; k2++) s += d2[k2] * W2[k2][j]; d1.push((1 - f.a1[j] * f.a1[j]) * s); }
      for (j = 0; j < h1; j++) { gb1[j] += d1[j]; for (i = 0; i < nIn; i++) gW1[j][i] += d1[j] * x[i]; }
    }
    var st = lr / N;
    var i2, j2, k3;
    for (i2 = 0; i2 < nIn; i2++) { b4[i2] -= st * gb4[i2]; for (j2 = 0; j2 < h1; j2++) W4[i2][j2] -= st * gW4[i2][j2]; }
    for (j2 = 0; j2 < h1; j2++) { b3[j2] -= st * gb3[j2]; for (k3 = 0; k3 < h2; k3++) W3[j2][k3] -= st * gW3[j2][k3]; }
    for (k3 = 0; k3 < h2; k3++) { b2[k3] -= st * gb2[k3]; for (j2 = 0; j2 < h1; j2++) W2[k3][j2] -= st * gW2[k3][j2]; }
    for (j2 = 0; j2 < h1; j2++) { b1[j2] -= st * gb1[j2]; for (i2 = 0; i2 < nIn; i2++) W1[j2][i2] -= st * gW1[j2][i2]; }
  }

  var errs = [];
  for (var i = 0; i < feats.length; i++) {
    var f = forward(feats[i]);
    var s = 0;
    for (var j = 0; j < nIn; j++) s += (f.out[j] - feats[i][j]) * (f.out[j] - feats[i][j]);
    errs.push(s / nIn);
  }
...
function flagTop(scores, contamination) {
  ...
  var idx = Math.max(0, Math.floor(contamination * sorted.length) - 1);
  var th = sorted[idx] || 101;
...
function findDuplicates(records) {
  ...
  var dupNum = [], dupPair = [], dupAmt = [];
  Object.keys(byNum).forEach(function (k) { if (byNum[k].length > 1) dupNum.push(byNum[k]); });
  Object.keys(byPair).forEach(function (k) { if (byPair[k].length > 1) dupPair.push(byPair[k]); });
  Object.keys(byAmt).forEach(function (k) { if (byAmt[k].length > 2) dupAmt.push(byAmt[k]); });
  return { dupNum: dupNum, dupPair: dupPair, dupAmt: dupAmt };
}

function seqGaps(records) {
  var nums = [];
  for (var i = 0; i < records.length; i++) {
    var n = parseInt(String(records[i].id).replace(/\D/g, ""), 10);
  ...
  for (var i = 1; i < nums.length; i++) {
    if (nums[i] - nums[i - 1] > 1) {
      gaps.push({ from: nums[i - 1], to: nums[i], missing: nums[i] - nums[i - 1] - 1 });
    }
  }
...
function simpleSample(N, n) {
  ...
  var out = Object.keys(s).map(Number);
...
console.log("✅ ديوان المُراجِع — تم التحميل بنجاح");
console.log("   sum:", typeof window.AuditorTools.sum);
console.log("   tripleScan:", typeof window.AuditorTools.tripleScan);